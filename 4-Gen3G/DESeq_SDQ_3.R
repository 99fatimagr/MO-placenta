...
metadata=metadata[!is.na(metadata$score_SDQ_total_3y),]
...
metadata$score_SDQ_total_3y=scale(metadata$score_SDQ_total_3y,center=T,scale=T)
metadata$niveau_scol_3y=as.character(metadata$niveau_scol_3y)

library(DESeq2)
dds <- DESeqDataSetFromMatrix(countData = RNA_seq_data,
                              colData = metadata,
                              design= ~offspring_sex+Patient_age_calc_V1+Accouchement_Mode+
                              Sample_Kit+niveau_scol_3y+score_SDQ_total_3y)

dds$niveau_scol_3y <- relevel(dds$niveau_scol_3y, ref = "4")
...
saveRDS(dds,"maternal_side_total_SDQ.rds")

resultsNames(dds)
res <- results(dds, name="score_SDQ_total_3y")
...
write.csv(res_gene,"maternal_side_SDQ_total.csv")

...
metadata=metadata[!is.na(metadata$score_ASEBA_5y),]
...
metadata$score_ASEBA_5y=scale(metadata$score_ASEBA_5y,center=T,scale=T)
...
dds <- DESeqDataSetFromMatrix(countData = RNA_seq_data,
                              ...
                              Sample_Kit+niveau_scol_5y+score_ASEBA_5y)
...
dds <- DESeq(dds)
saveRDS(dds,"maternal_side_total_ASEBA_5.rds")

resultsNames(dds)
res <- results(dds, name="score_ASEBA_5y")
...
write.csv(res_gene,"maternal_side_ASEBA_5_total.csv")
